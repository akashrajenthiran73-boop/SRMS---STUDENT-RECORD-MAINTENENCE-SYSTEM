...
// APPROVE LOGIC (Using student_id or id)
if(isset($_GET['approve_id']) && !empty($_GET['approve_id'])){
    $approve_id = $_GET['approve_id']; 
    
    // student_id vaithu Supabase Update
    $url = rtrim($SUPABASE_URL, '/') . "/rest/v1/umis_students?student_id=eq." . urlencode($approve_id);
    $data = json_encode(['status' => 'Approved', 'is_completed' => true]); 
    ...
    if($http_code >= 200 && $http_code < 300){
        header("Location: umis_form_list.php?msg=approved"); 
        exit;
    } else {
        $error_msg = "Database Update Failed! Code: $http_code";
    }
}
...
if (!empty($SUPABASE_URL) && !empty($SUPABASE_KEY)) {
    $url = rtrim($SUPABASE_URL, '/') . "/rest/v1/umis_students?select=*&order=created_at.desc";
    $ch = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true, 
        CURLOPT_SSL_VERIFYPEER => false,
...
<title>UMIS Data Management - HOD Portal</title>
...
.status-pill.pending, .status-pill.in { background: #FEF3C7; color: #B45309; border: 1px solid #FDE68A; }
    ...
    <div class="content-body">
        <div class="page-header">
            <div class="page-header-title">
                ...
                <div>
                    <h1>UMIS Cards & Approvals</h1>
                    <p>Total Records: <b><?=count($umis_list)?></b> student applications</p>
                </div>
            </div>
        ...
        <?php if(isset($_GET['msg'])): ?>
            <div class="msg"><i class="fa-solid fa-circle-check"></i> UMIS Card Approved Successfully!</div>
        <?php endif; ?>

        <?php if(!empty($error_msg)): ?>
            <div class="err"><i class="fa-solid fa-circle-exclamation"></i> <?=htmlspecialchars($error_msg)?></div>
        <?php endif; ?>
        ...
        <div class="table-card">
            <div class="table-responsive">
                <table id="umisTable">
                    ...
                    <tbody>
                    <?php if(is_array($umis_list) && count($umis_list) > 0): $i=1; foreach($umis_list as $u): 
                        $raw_status = $u['status'] ?? 'Pending';
                        ...
                        $status_lower = strtolower($clean_status);
                        $initial = strtoupper(substr($name, 0, 1));
                    ?>
                    <tr>
                        <td style="font-weight: 600; color: #94A3B8;"><?=$i++?></td>
                        <td><span class="id-badge"><?=htmlspecialchars($student_id)?></span></td>
                        <td>
                        ...
                        <td>
                            <span class="status-pill <?=$status_lower?>">
                                <i class="fa-solid fa-circle" style="font-size: 6px;"></i> <?=htmlspecialchars($clean_status)?>
                            </span>
                        </td>
                        <td>
                            <?php if($status_lower == 'pending' || $status_lower == 'in progress'): ?>
                                <a href="umis_form_list.php?approve_id=<?=urlencode($student_id)?>" class="btn-action btn-approve" onclick="return confirm('Approve UMIS for <?=htmlspecialchars($name)?>?')"><i class="fa-solid fa-check"></i> Approve</a>
                            <?php else: ?>
                                <span class="approved-tag"><i class="fa-solid fa-circle-check"></i> Approved</span>
                            <?php endif; ?>
                            <a href="umis_view.php?student_id=<?=urlencode($student_id)?>" class="btn-action btn-view"><i class="fa-solid fa-eye"></i> View</a>
                        </td>
                    </tr>
                    <?php endforeach; else: ?>
                    <tr>
...
<script>
// Live search filter
document.getElementById('umisSearch')?.addEventListener('keyup', function() {
    const filter = this.value.toLowerCase();
    const rows = document.querySelectorAll('#umisTable tbody tr');
    rows.forEach(row => {
