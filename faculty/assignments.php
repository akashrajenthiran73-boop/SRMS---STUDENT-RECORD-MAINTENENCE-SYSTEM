...
// Handle Assignment Post Action (Faculty Only)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_assignment') {
    ...
    // Handle File Upload (Optional Reference Question Paper/Instructions)
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        $new_filename = time() . '_assign_' . preg_replace('/[^a-zA-Z0-9_\.-]/', '_', $_FILES['file']['name']);
    ...
    $url = rtrim($SUPABASE_URL, '/') . "/rest/v1/assignments";
    $ch = curl_init($url);
...
// Delete Assignment Action (Faculty Only)
if (isset($_GET['action']) && $_GET['action'] === 'delete' && $is_faculty) {
    $del_id = $_GET['id'] ?? '';
    if (!empty($del_id)) {
        $url = rtrim($SUPABASE_URL, '/') . "/rest/v1/assignments?id=eq." . urlencode($del_id);
        $ch = curl_init($url);
...
if (!empty($SUPABASE_URL) && !empty($SUPABASE_KEY)) {
    $fetch_url = rtrim($SUPABASE_URL, '/') . "/rest/v1/assignments?select=*&order=id.desc";
    ...
    $res = curl_exec($ch);
    curl_close($ch);
    $assignments = json_decode($res, true) ?? [];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Assignments to Students - Faculty Portal</title>
...
.badge-due { background: #FEF2F2; color: #DC2626; border: 1px solid #FEE2E2; padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; }

.btn-dl { background: linear-gradient(135deg, #2563EB, #1D4ED8); color: white; padding: 6px 12px; border-radius: 7px; text-decoration: none; font-size: 11.5px; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; transition: 0.2s; }
...
<div class="sidebar">
    <div class="sidebar-top">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="assignments.php" class="active"><i class="fa-solid fa-tasks"></i> Assignments</a></li>
...
<div class="main-content">
    <div class="topbar">
        <div class="topbar-title">
            <h1>Assignments to Students</h1>
            <p>Publish homework, term papers, submission deadlines, and problem sets</p>
        </div>
        ...
        <div class="card">
            ...
            <form action="assignments.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" placeholder="e.g. Assignment 1 - Memory Management" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" placeholder="Write questions or instructions here..."></textarea>
                </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <span style="font-size: 12px; color: #64748B; font-weight: 500;">
                    Total: <?php echo count($assignments); ?> assignments
                </span>
            </div>
                ...
                <table class="sub-table">
                    ...
                    <tbody>
                        <?php if (empty($assignments)): ?>
                            <tr>
                                <td colspan="4" style="text-align: center; color: #94A3B8; padding: 32px 16px;">
                                    <i class="fa-regular fa-clipboard" style="font-size: 26px; display: block; margin-bottom: 8px; color: #CBD5E1;"></i>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td>
                                    <strong style="color: #0F172A;"><?php echo htmlspecialchars($item['title'] ?? 'N/A'); ?></strong>
                                    <?php if (!empty($item['description'])): ?>
                                        <p style="font-size: 12.5px; color: #64748B; margin-top: 4px; line-height: 1.4;"><?php echo nl2br(htmlspecialchars($item['description'])); ?></p>
                                    <?php endif; ?>
                                </td>
                                <td><span style="font-weight:700; color:#475569;"><?php echo htmlspecialchars($item['subject_code'] ?? 'N/A'); ?></span></td>
                                <td>
                                    <span class="badge-due">
                                        <i class="fa-regular fa-calendar-xmark"></i> <?php echo htmlspecialchars($item['due_date'] ?? 'N/A'); ?>
                                    </span>
                                </td>
                                <td>
                                    ...
                                    <?php endif; ?>
                                    <?php if ($is_faculty): ?>
                                        <a href="assignments.php?action=delete&id=<?php echo $item['id']; ?>" class="btn-del" onclick="return confirm('Delete this assignment?')">
